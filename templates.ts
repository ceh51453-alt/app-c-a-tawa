...
### 8. ST-PROMPT-TEMPLATE (EJS TEMPLATING NÂNG CAO)
Extension này cho phép nhúng mã JavaScript qua các thẻ EJS \`<% %>\`, \`<%= %>\`, và \`<%- %>\`.
Các Decorators đặc biệt (phải nằm ở dòng đầu tiên của entry, không có dòng trống ở giữa):
- \`@@preprocessing\`: Trình tiền xử lý. Nếu EJS trả về khoảng trắng hoặc comment \`<!-- ẩn -->\`, entry sẽ không được đưa vào prompt. Dùng để tối ưu hóa bộ nhớ dựa trên vị trí/biến số.
- \`@@generate_before\`: Chạy EJS trước khi bắt đầu sinh prompt.
- \`@@generate_after\`: Chạy EJS sau khi sinh prompt.
- \`@@private\`: Đảm bảo các biến cục bộ khai báo trong EJS của entry này không làm nhiễm độc các entry khác.

Các hàm tiện ích có sẵn trong EJS context (luôn đi kèm \`await\` nếu là hàm bất đồng bộ):
* Quản lý biến:
  - \`getvar(key, options)\`: Đọc biến. Ví dụ: \`getvar('stat_data.Nhân_vật.HP')\`. Hỗ trợ: \`getvar('path', { defaults: defaultValue })\`.
  - \`setvar(key, value)\`: Ghi biến. Ví dụ: \`setvar('stat_data.Nhân_vật.HP', 100)\`.
  - \`incvar(key, value, options)\`: Tăng giá trị biến.
  - \`decvar(key, value, options)\`: Giảm giá trị biến.
  - \`delvar(key, index, options)\`: Xóa một phần tử trong mảng hoặc thuộc tính của object.
  - \`insvar(key, value, index, options)\`: Chèn một phần tử vào mảng/object.
  - \`define(name, value, merge)\`: Định nghĩa biến toàn cục.
  - \`patchVariables(key, changes, options)\`: Cập nhật biến hàng loạt bằng JSON Patch.
* Truy xuất World Info (Lorebook):
  - \`await getwi(title)\`: Đọc và biên dịch nội dung của một entry khác theo tiêu đề/uid.
  - \`await activewi(title, force)\`: Kích hoạt động một entry. Nếu \`force\` là true, bỏ qua kiểm tra keyword.
  - \`getEnabledWorldInfoEntries()\`: Lấy danh sách tất cả các entry đang được kích hoạt.
* Nhân vật và Preset:
  - \`await getchar(name)\`: Đọc thông tin thiết lập (description, personality...) của nhân vật.
  - \`await getCharData(name)\`: Lấy dữ liệu thô của nhân vật.
* Chat History:
  - \`getChatMessage(idx, role)\`: Lấy tin nhắn. Chỉ số âm tính từ cuối chat (ví dụ: -1 là tin nhắn mới nhất).
  - \`getChatMessages(count)\` hoặc \`getChatMessages(start, end, role)\`: Lấy danh sách tin nhắn.
  - \`matchChatMessages(pattern, options)\`: Kiểm tra xem các tin nhắn gần đây có khớp regex không.
* Prompt Injection & Dynamic UI:
  - \`injectPrompt(key, prompt, order, sticky, uid)\`: Chèn động một prompt tạm thời vào context.
  - \`activateRegex(pattern, replace, opts)\`: Kích hoạt động một regex script.
  - \`await evalTemplate(content, data)\`: Tự biên dịch một chuỗi chứa cú pháp EJS khác.

### 9. JS-SLASH-RUNNER (TAVERN HELPER SCRIPTS)
Extension này chạy Client-side JavaScript trong trình duyệt, cho phép tạo giao diện động tương tác với SillyTavern qua \`window.TavernHelper\` hoặc \`Mvu\`.
Các hàm và cơ chế chính:
- **Chờ khởi tạo**: Luôn bắt đầu bằng \`await waitGlobalInitialized('Mvu')\` để đảm bảo MVU engine đã sẵn sàng.
- **Lắng nghe sự kiện**:
  - \`eventOn(event_type, listener)\`: Đăng ký lắng nghe sự kiện lâu dài.
  - \`eventOnce(event_type, listener)\`: Đăng ký lắng nghe một lần.
  - Các sự kiện quan trọng:
    - \`Mvu.events.VARIABLE_UPDATE_ENDED\` (khi biến số được AI cập nhật xong): \`eventOn(Mvu.events.VARIABLE_UPDATE_ENDED, (new_vars, old_vars) => { ... })\`.
    - \`Mvu.events.COMMAND_PARSED\` (khi parse xong lệnh cập nhật): cho phép can thiệp/sửa đổi lệnh trước khi lưu.
    - \`iframe_events.MESSAGE_IFRAME_RENDER_STARTED\` / \`MESSAGE_IFRAME_RENDER_ENDED\`.
- **Đọc/Ghi biến số**:
  - \`getAllVariables()\`: Lấy toàn bộ biến số hiện tại.
  - \`Mvu.getMvuData({ type: 'message', message_id: idx })\`: Lấy data biến ở một tin nhắn cụ thể.
  - \`Mvu.replaceMvuData(new_data, { type: 'message', message_id: idx })\`: Ghi đè lại data biến.
- **Tương tác parent và slash command**:
  - \`triggerSlash(command)\`: Gọi lệnh slash. Ví dụ: \`triggerSlash('/sys Mưa rơi...')\`, \`triggerSlash('/trigger')\` (bắt AI phản hồi).
- **Âm thanh và SFX**:
  - \`playAudio(url, options)\`: Phát nhạc/sfx (ví dụ: \`playAudio('link.mp3', { volume: 0.5 })\`).
  - \`stopAudio(audio)\`.
...
### 10. SỬ DỤNG TẤT CẢ SỨC MẠNH CỦA JS-SLASH-RUNNER (TAVERN HELPER SCRIPTS)
- Khi viết mã JavaScript trong UI, bạn có thể gọi:
  - \`triggerSlash('/sys Nội dung')\` để thông báo cho người dùng hoặc thêm nhật ký hệ thống.
  - \`triggerSlash('/trigger')\` để ra lệnh cho AI phản hồi ngay lập tức (phù hợp khi người dùng click vào các nút hành động trên UI).
  - \`playAudio('https://example.com/sound.mp3', { volume: 0.8 })\` để chơi hiệu ứng âm thanh sống động khi người dùng tăng cấp, nhận quà, hoặc chiến đấu.
  - Sử dụng sự kiện \`Mvu.events.VARIABLE_UPDATE_ENDED\` để đồng bộ hóa giao diện UI ngay khi có cập nhật biến từ LLM.
- **Ví dụ nâng cao tương tác**:
  \`\`\`javascript
  async function performAction() {
    // 1. Chơi âm thanh click
    playAudio('https://api.mixkit.co/active.mp3');
    // 2. Chạy lệnh slash cập nhật biến qua MVU hoặc gửi tin nhắn hệ thống
    await triggerSlash('/sys Bạn vừa bấm nút nâng cấp kiếm!');
    await triggerSlash('/mvu set chu_nhan/tui_do/Kiem_Sat/so_luong 1');
    // 3. Ra lệnh AI phản hồi
    await triggerSlash('/trigger');
  }
  $('#btn-upgrade').off('click').on('click', errorCatched(performAction));
  \`\`\`
</REGEX_UI_DESIGN_RULES>
...
<EJS_RULES>
1. Truy cập biến: Sử dụng \`_.get(stat_data, 'Đường.Dẫn.Biến', 'Giá trị mặc định')\` hoặc \`getvar('stat_data.Đường.Dẫn.Biến', { defaults: 'Giá trị mặc định' })\` để đọc dữ liệu từ biến MVU.
2. Cú pháp EJS:
...
3. Lọc entry: Dùng tag \`@@preprocessing\` ở dòng đầu tiên của lorebook entry, theo sau là khối EJS để lọc nạp.
   ...
   <!-- ẩn -->
   <% } %>
4. Các hàm ST-Prompt-Template nâng cao (Luôn dùng \`await\` cho hàm bất đồng bộ):
   - \`getvar('path')\`: Đọc biến.
   - \`setvar('path', value)\`: Ghi biến.
   - \`incvar('path', delta)\` / \`decvar('path', delta)\`: Tăng/giảm biến.
   - \`await getwi('Tên Entry')\`: Đọc nội dung entry khác.
   - \`await activewi('Tên Entry', true)\`: Ép buộc kích hoạt entry khác.
   - \`await getchar()\` / \`await getCharData()\`: Lấy dữ liệu nhân vật hiện tại.
   - \`getChatMessage(-1)\`: Lấy tin nhắn cuối cùng (ví dụ: \`getChatMessage(-1).message\` để đọc text).
   - \`getChatMessages(5)\`: Lấy 5 tin nhắn cuối cùng.
</EJS_RULES>
